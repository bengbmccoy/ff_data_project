...
# 2. Check the json files have the right schema
# 3. Write a test that checks the schema of create_results_dateframe against JSON filess
...
def main(year=2024, type="REG", verbose=False, prod=False):
    ...
    json_files, path_to_json = get_json_files()
    ...
    results_df = unpack_json(json_files, path_to_json, results_df)
...
def unpack_json(json_files, path_to_json, results_df):
    ...
    for i in json_files:
        with open(os.path.join(path_to_json, i)) as f:
            ...
            # Unpacking and flattening logic
            row_year = data['year']
